...
def run_synthesis(model, extra_data=None, all=False):
    ...
    for patch in patch_files:
        os.remove(str(utils.ROOT_PATH / f'{patch}.patch'))

    data_list = get_data(extra_data=extra_data)

    save_dir = utils.ROOT_PATH / 'results'
    if save_dir.exists():
